...
from annfore.learn.train_params import learn_two_gamma
...
def add_arg_parser(parser):
    ...
    parser.add_argument('--beta_start_learn',  type=float, default=0.5, dest="beta_start_learn", help="value of beta where the learning of epidemic params starts")
    ...
    parser.add_argument('--gamma1_init_param', type=float, default=1e-3, dest="gamma1_init_param", help="gamma1 starting value of the learning")
    parser.add_argument('--gamma2_init_param', type=float, default=1e-3, dest="gamma2_init_param", help="gamma2 starting value of the learning")
    parser.add_argument('--gamma1', type=float, default=1e-3, dest="gamma1", help="gamma1")
    parser.add_argument('--gamma2', type=float, default=1e-3, dest="gamma2", help="gamma2")

    parser.add_argument('--fraction_nodes1', type=float, default=0.5, dest="fraction_nodes1", help="fraction_nodes1")


    parser.add_argument("--exp_like_beta",action="store_true", help="beta annealing with more exponential like sequence of beta")
    parser.add_argument("--rand_split_nodes", action="store_true", help="split node random in inference")
...
## ************ RUN INFERENCE ALGORITHMS ************
    ...
    for instance_num in range(args.start_conf, args.num_conf):
        ...
        opt_params=[gamma1_opt, gamma2_opt]
        ...
        if args.rand_split_nodes:
            ...
            split_nodes=int(args.N*args.fraction_nodes1)
            list_N = np.arange(args.N)
            rnd_gen.shuffle(list_N)
            nodes_1, nodes_2 = list_N[:split_nodes], list_N[split_nodes:]
        
        model.nodes_1 = nodes_1
